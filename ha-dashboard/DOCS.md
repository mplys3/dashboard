...
2. Start the add-on once to let it create `/config/ha-config.js` from the bundled dashboard configuration.
3. Edit the generated `ha-config.js` in Home Assistant's `/addon_configs/...` folder if needed.
...
On first start, the add-on creates `/config/ha-config.js` automatically if it does not already exist. Home Assistant exposes that file on the host under `/addon_configs/...`. Edit that file to customize rooms, entities, Spotify settings and system status sensors.
